app.controller("CategoryEditController", function ($scope, $http) {
    ...
    $http({
        method: "GET",
        headers: GetHeader(),
        url: "https://localhost:7019/api/Category/Get?categoryId=" + $scope.GetUrlParameter("Id")
    ...
    $scope.CategoryUpdate = function () {
        $http({
            method: "PUT",
            headers: GetHeader(),
            url: "https://localhost:7019/api/Category/Update?categoryId=" + $scope.GetUrlParameter("Id"),
            data: $scope.category
        }).then(function (response) {
